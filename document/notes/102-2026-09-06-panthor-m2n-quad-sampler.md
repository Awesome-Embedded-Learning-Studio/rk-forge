...
4. **受控矩阵 9/9 PASS**（2..256 全屏 + 4 子区域含 0.25..0.75）+
   gt/gc 回归绿。
5. **screendump 首现非 GPU 路径可解释的内容**：640×480 出现 701
   像素 0xaaaaaa 文字行（x∈[200,438] y∈[226,238]，恰在 512² 图集
   坐标域内）。**来源未验证**——本 boot 主 FBO 零 GPUBLIT/GPUFBG
   （全被 raster 消费），文字可能是图集被部分 scanout 或别的链；
   不得据此宣称"桌面可见已通"（note 76 证据门）。壁纸已 raster
   进自身 3840×2160 bf=13 缓冲。
