"""Board config — the single source of truth per board (boards/<id>/board.yaml).
...
class Board:
    ...
    bringup_dir: str = ""
    board_cfg_dir: str = ""
    sources: dict = field(default_factory=dict)   # §5.1: per-board upstream refs (linux/uboot/openwrt)
    ...
    def from_yaml(cls, board_id: str, root) -> "Board":
        """Load ``boards/<board_id>/board.yaml`` → ``Board``, then validate."""
        path = Path(root) / "boards" / board_id / "board.yaml"
        ...
        board = cls(
            ...
            sources=dict(raw.get("sources", {})),   # §5.1 per-board upstream refs
            # rkbin
    ...
    def to_bash_env(self) -> str:
        ...
        lines: list[str] = [
            ...
            f" — source: boards/{self.id}/board.yaml. Do not edit."
        ]
