...
用法(免 root):
  uv run --with pypandoc-binary --with weasyprint python3 document/pdf/build_pdf.py
"""
import re
import sys
...
ROOT = _HERE.parents[2]              # document/pdf/ → repo root
DOC = ROOT / 'document'
OUT = ROOT / 'dist-pdf' / 'rk-forge.pdf'
CSS_FILE = _HERE.parent / 'book.css'   # sits next to this script

# ── 封面元信息(固化在脚本顶部;纯 Python,不依赖 project.config.ts)────────────
META = {
    'eyebrow': 'RK-Forge · 整站教程 PDF',
    'title': 'RK-Forge 的教程文档',
    'subtitle': '专注于 Rockchip RK3506 主线优先的嵌入式 Linux 教程',
    'author': 'Awesome-Embedded-Learning-Studio · Charliechen114514',
}
...
# 与 sidebar.ts 保持一致
LEARNING_ORDER = ['boot', 'rootfs', 'peripherals', 'sd-boot', 'forge']
VOLUMES = ['tutorial', 'pitfalls', 'notes']  # 阅读顺序(对应 nav:教程/踩坑/笔记)
VOLUME_TITLE = {'tutorial': '教程', 'pitfalls': '踩坑日记', 'notes': '工程笔记'}
...
def ordered_files():
    ...
    # 附录:根级 sdk-diff / blobs(若存在)
    for extra in ['sdk-diff.md', 'blobs.md']:
        p = DOC / extra
        if p.exists():
            files.append(p)
    return files
...
VUE_WRAP_LINE = re.compile(
    ...
    r'DocNavCards|StatusTag|StepFlow|StepItem|InfoCard|ChapterNav|ChapterLink)\b[^>]*/?>\s*$')
...
RESIDUAL_COMP = re.compile(
    ...
    r'DocNavCards|StatusTag|StepFlow|StepItem|InfoCard|ChapterNav)\b[^>]*/?>')
...
def main():
    ...
    # 组装:卷首插 part 标题;每篇前插分页符。pandoc 保留 raw HTML 的 <div>。
