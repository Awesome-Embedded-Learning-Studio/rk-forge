 ...
 *   - options.config(md) 在 VitePress 把 Shiki/复制按钮/行号/preWrapper 全部接好
 *     【之后】才执行(所有 md.use() 链之后),故此处捕获的 md.renderer.rules.fence
 *     已是完整链 —— 调用它即得含 .copy/.lang/pre.shiki/.line-numbers-wrapper 的
 *     整段 HTML。直接整段包 <details>,内部兄弟链不动。
 *   - 必须整段包(而非只包 <pre>):VitePress 复制按钮靠 button.copy.nextElementSibling
 *     定位 <pre>,兄弟链一断复制即失效。
 *   - 代码放在 <details>【外面】(.vp-code-fold 里与 <details> 同级),而非放进
 *     <details> 内:原生 <details> 关闭时不渲染非 summary 内容;放外面则代码始终
 *     在 DOM 中,纯 CSS :has(details[open]) 控制 display(收起完全隐藏、不做预览;
 *     展开全显示)—— 零 JS、无 JS 也能点 summary 展开、@media print 强制显示即
 *     完整打印。
 *   - code-group 内的 fence 不折叠(tab 本身已是折叠语义):core ruler 用独立 depth
 *     计数打 token.meta.inCodeGroup 标记 —— 不依赖 render 期才追加的 " active" info。
 *
 * 阈值 20:20 行以内的中等块保持原样、不误伤阅读,超过的(多为演示性大段输出或完整
 * 工程)才折叠。改这一处常量即可全局调档。
 */
