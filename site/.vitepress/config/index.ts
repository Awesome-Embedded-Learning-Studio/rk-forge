...
export default defineConfig({
  ...
  cleanUrls: true,
  lastUpdated: true,
  // github-links.ts 已把"跳出 srcDir 的仓库源文件/非页面资源"重写成 GitHub URL,
  // 这里再兜底:万一有漏网的相对链接指向仓库根(非 document/ 内页面),不当死链报错。
  ignoreDeadLinks: [
    /\/(?:patches|board|pins|scripts)\//,
    /\/(?:BLOBS|PLAN|CHANGELOG|QUICK_START|CONTRIBUTING)\.md$/,
    /\.(?:txt|patch|sh|py|its|ini|cfg|conf)$/,
  ],
