...
> **2026-08-29 后续产品裁决**：本任务书第 2 节的 virtio 禁令已被用户撤销。
> 现行边界是“真机与仿真使用同一输入 DTB，不做 sim overlay、不注入外设节点”；
> virtio 仅由仿真 bootargs 枚举，真机 bootargs 不含这些参数。见笔记 77。本任务书
> 其余 Mali/CSF 研究内容仍可作为长期真 GPU 建模线使用。
