...
这一层是 rk-forge 的**诚实底层**：57 篇带日期的 bring-up 流水，原样保留——成功的、失败的、brick 时的慌乱、半成品、噪音，一个不删、不去重。它是踩坑日记（[pitfalls/](../pitfalls/)）和教程（[tutorial/](../tutorial/)）的取证源：后两者是提炼后的结论，这里是现场。
