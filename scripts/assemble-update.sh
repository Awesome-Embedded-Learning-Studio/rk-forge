...
# partition images, using the forge Python packer (scripts/rkfw-pack.py).
#
...
# (rkfw-pack.py unpack) and verifies the expected partitions are present with
# matching sizes. No board needed to trust the pipeline.
#
# Usage:
...
# Pack RKAF+RKFW via the forge Python packer (replaces vendor afptool + rkImageMaker).
# Chip tag is read from the loader inside rkfw-pack.py (same dd-skip-21 | rev method
# → RK350F); the variant is just which partition images were staged into ROCKDEV above.
log_info "rkfw-pack.py pack (RKAF+RKFW, variant=$VARIANT)…"
python3 "${_SCRIPT_DIR}/rkfw-pack.py" pack "$PKGFILE" "$ROCKDEV/Image" \
  "$LOADER" "$PARAMETER" "$UPDATE_OUT" \
  || die "rkfw-pack.py pack failed"
[[ -f "$UPDATE_OUT" ]] || die "rkfw-pack.py produced no $UPDATE_OUT"
log_ok "$(basename "$UPDATE_OUT") → $UPDATE_OUT ($(stat -c%s "$UPDATE_OUT") B, variant=$VARIANT)"

# --- round-trip self-check (board-independent, vendor-free) ---------------
if [[ "$VERIFY" == 1 ]]; then
  log_info "round-trip self-check (rkfw-pack.py unpack own output)…"
  VFY=$(mktemp -d)
  ...
  for pair in "${PAIRS[@]}"; do
    p=${pair%%:*}; src=${pair#*:}
    [[ -f "$VFY/$p" ]] || die "self-check FAIL: $p missing after unpack"
    a=$(stat -c%s "$VFY/$p"); b=$(stat -c%s "$src")
    [[ "$a" == "$b" ]] || die "self-check FAIL: $p size $a != source $b"
  done
  log_ok "self-check OK: $(basename "$UPDATE_OUT") round-trips (uboot/boot${ROOTFS:+/rootfs} sizes match)"
fi
