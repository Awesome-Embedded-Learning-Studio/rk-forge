...
// 右栏 --vp-aside-width 自定义变量(在 interactive.css 覆盖 aside max-width);右 handle absolute
...
const CONF: Record<Side, Dim> = {
  left: { min: 240, max: 480, def: 272, key: 'vp-sidebar-width-v2', cssVar: '--vp-sidebar-width' },
...
function startDrag(side: Side, e: MouseEvent) {
  ...
  // 位置,按绝对坐标算会多算居中偏移导致宽度暴涨。位移法与布局无关,恒正确。
...
function reset(side: Side) {
  if (side === 'left') {
    document.documentElement.style.removeProperty(CONF.left.cssVar)
    try { localStorage.removeItem(CONF.left.key) } catch {}
    updateLeftPosition()
    return
  }
...
<template>
  <!-- 初始隐藏:无侧栏页(首页)上 SSR HTML 也带本元素,水合前它是一条悬在
       内容上的 1px 竖线;updateLeftVisibility 在有侧栏时会清掉这条 inline
       style 恢复显示,无侧栏则保持隐藏。 -->
  <div
    ref="leftHandle"
